...
// Обработчик сброса ингредиентов
document.getElementById('reset-ingredients').addEventListener('click', () => {
    ...
    selectedIngredients.length = 0; // Очищаем массив ингредиентов
    cup.innerHTML = "<p>Перетащите сюда ингредиенты</p>"; // Сбрасываем визуально
    document.getElementById("score").textContent = `⭐ Очки: ${game.score}`; // Обновляем счет
});
